...
--paste/ copy from sys clipboard unless specified otherwisee
...
require("mason-lspconfig").setup({
	ensure_installed = { "lua_ls", "rust_analyzer", "clangd", "jdtls", "html", "cssls", "pyright", "ts_ls" },
})
...
vim.keymap.set("n", "<leader>td", function()
	local bufnr = 0
	local enabled = vim.b[bufnr].diagnostics_disabled ~= true
	if enabled then
...
end, { desc = "Toggle LSP diagnostics (buffer)" })
